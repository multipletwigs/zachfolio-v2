          ...
          <>
            <Popover.Button
              className={`
                ...
                group inline-flex h-10 w-10 items-center rounded-lg bg-slate-200 text-slate-600 dark:bg-slate-500 px-3 py-2 dark:text-slate-700 hover:text-opacity-100 focus:outline-none focus-visible:ring-2 focus-visible:ring-white focus-visible:ring-opacity-75`}
            >
              <GiHamburgerMenu size="1.5em" />
            </Popover.Button>
            <Popover.Panel className="absolute z-10 mt-3 w-60 -translate-x-[180px] transform px-4">
              <div className="flex flex-col rounded-lg shadow-lg ring-1 ring-black ring-opacity-5 dark:bg-slate-900 bg-slate-100 py-3">
                <NavItem href={'/'} navTitle={'Home'} />
                <NavItem href={'/blog'} navTitle="Blogs" />
                <NavItem href={'/about'} navTitle="About Me" />
                <hr className=""></hr>
                <div className="flex flex-row align-middle justify-center mt-3">
                    <ThemeToggle/>
                </div>
              </div>
            </Popover.Panel>
...
const NavItem = ({ href, navTitle }: NavItemType) => {
  ...
  return (
    <NextLink className={'p-10 hover:bg-slate-500 dark:hover:bg-slate-50'} href={href}>
      <a
        className={`${
          isActive
            ? 'p-3 font-bold hover:bg-slate-50/10'
            : 'p-3 font-normal text-gray-400 hover:bg-slate-50/10'
        }`}
      >
        <span
          className={`${
